...
public class SortDescHandler implements ContextHandler {
    ...
    public boolean shouldRun(ProcessingContext context) {
        return context.getUserState().getState()== UserState.State.WAITING_FOR_SORTING_TYPE_ASCDESC
                && context.getUpdate().getMessage().getText().equals("По убыванию");
    }

    @Override
    public void execute(ProcessingContext context) {
        context.getUserState().setOrderTypeAscDesc(TelegramBotRESTHandler.OrderTypeAscDesc.DESC);
